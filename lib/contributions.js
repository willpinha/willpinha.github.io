...
export function buildContributionGraph(calendar) {
	...
	// Rects are inset by half the stroke so the 1px border lands on whole pixels
	const inset = cellStroke / 2;
	...
	calendar.weeks.forEach((week, weekIndex) => {
		...
		for (const day of week) {
			cells.push({
				x: x + inset,
				y: topPad + day.date.getUTCDay() * step + inset,
				level: day.level,
